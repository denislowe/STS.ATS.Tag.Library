(function(wnd) {
    ...
    contentLoaded(wnd, function() {
        ...
        try {
            ...
            var url = "https://click.appcast.io/tee-9k/a17.png?r=" +
                encodeURIComponent(document.referrer) + (reqNum ? "&jid=" + reqNum : "") + "&tn=" + now + "&rn=" + random + "&fp=" + fp + '&e=1438';
    ...
    function getReqNum() {
        var reqNum = null,
            jobContainer = document.getElementById("contentcontainer");
        if (jobContainer) {
            var elements = jobContainer.getElementsByTagName("div");
            for (var i = 0; i < elements.length; i++)
                if (elements[i].className && elements[i].className.split(" ").indexOf("jobid") !=
                    -1) {
                    var jobEl = elements[i].getElementsByTagName("div");
                    for (var j = 0; j < jobEl.length; j++)
                        if (jobEl[j].className && jobEl[j].className.split(" ").indexOf("jobdescription-value") != -1) reqNum = jobEl[j].innerHTML
                }
        }
        if (!reqNum && sessionStorage && sessionStorage.getItem("jobId")) reqNum = sessionStorage.getItem("jobId");
        return reqNum
    }
