(function(wnd) {
    ...
    contentLoaded(wnd, function() {
        ...
        try {
            ...
            var url = "https://click.appcast.io/icims-9k/a17.png?r=" +
                encodeURIComponent(document.referrer) + (reqNum ? "&jid=" + reqNum : "") + "&tn=" + now + "&rn=" + random + "&fp=" + fp + '&e=800';
    ...
    function getJobSeekerId() {
        ...
        if (jobSeekerId) return jobSeekerId;
            ...
            var match = document.head.innerHTML.match(/\'application complete\', \'\w+\', (\w+)/i);
            if (match) return match[1];
            if (typeof _c2a !==
                "undefined")
                for (var i = 0; i < _c2a.length; i++)
                    if (_c2a[i][0] === "Apply") return _c2a[i][2];
            return "not_found"
        }
    }
