(function(wnd) {
    ...
    contentLoaded(wnd, function() {
        ...
        try {
            ...
            var url = "https://click.appcast.io/pplclck-9k/a17.png?r=" +
                encodeURIComponent(document.referrer) + (reqNum ? "&jid=" + reqNum : "") + "&tn=" + now + "&rn=" + random + "&fp=" + fp + '&e=2725';
    ...
    function getReqNum() {
        var jobId = null,
            match = null,
            jobInfoBlock = document.getElementsByName("jobApplicationForm");
        if (jobInfoBlock.length > 0) {
            var elements = jobInfoBlock[0].getElementsByTagName("span");
            for (var i = 0; i < elements.length; i++)
                if (elements[i].className == "pc-rtg-jobApp-jobAppliedTo") {
                    match = elements[i].innerText.match(/job id:\s(\w+)/im);
    ...
    function saveReqNumToSessionStorage(reqNum) {
        if (sessionStorage && reqNum) sessionStorage.setItem("jobId",
            reqNum)
    }
})(window);
